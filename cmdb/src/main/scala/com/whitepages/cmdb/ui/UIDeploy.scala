...
case class UIDeploy(ops: Neo4jOps, util: UIUtil) {
  ...
  case class DeployWindow(stackRef: JsonObject, clusterRef: JsonObject, serviceRef: JsonObject, deployments: JsonArray,
                          reload: () => Unit) extends Window("Deploy") {
    ...
    def vact(versionRef1: JsonObject) {
      ...
      val useRefs = if (versionRef == emptyJsonObject) {
        ...
        if (jsize(schema) > 0) {
          val uiDynamic = UIDynamic(ops, util)
          //uses.addComponent(new Button("Dynamic Properties"))
          val b = button("Set Dynamic", UI.getCurrent.addWindow(uiDynamic.DynamicWindow(version, stackRef, clusterRef, serviceRef, deployments, null)))
          uses.addComponent(b)
    ...
    val v = verticalBorder()(
      ...
      ySpace(),
      select1("Version", verRefs, vact),
      ySpace(),
      uses,
      (new Button("Advanced Options ...")),
      ySpace(),
      button("Deploy", dact)
    )
    setContent(v)
