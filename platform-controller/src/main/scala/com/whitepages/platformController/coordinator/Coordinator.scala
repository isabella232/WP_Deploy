...
class Coordinator(id: RequestId, factory: ActorRefFactory, serviceName: String, groupName: String, envName: String,
    ...
    if (map.size > 0) {
      ...
      p.future flatMap {
        case result =>
          ...
          deployOrdered(map1, serviceName, version, select, hasLB, domain)
      }
    } else {
      Future.successful(())
    }
  ...
  def deploy(serviceName: String, version: String, select: Seq[String], hasLB: Boolean, domain: String) {
    val f = downloadOrdered(hostMap, serviceName, version)
    val f1 = f flatMap {
      case x => deployOrdered(hostMap, serviceName, version, select, hasLB, domain)
    }
